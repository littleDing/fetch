...
FetchStream.prototype.parseUrl = function(url,reqOption){
	var urloptions = {}; for(var i in reqOption){urloptions[i]=reqOption[i];}
    var urlparts = urllib.parse(url, false, true),
        transport;
	urloptions.host = urlparts.hostname || urlparts.host;
	urloptions.port = urlparts.port;
	urloptions.path = urlparts.pathname + (urlparts.search || "") || "/";
	urloptions.method = this.options.method;
    /*    urloptions = {
...
FetchStream.prototype.runStream = function(url){
    var url_data = this.parseUrl(url,this.options.reqOption),
